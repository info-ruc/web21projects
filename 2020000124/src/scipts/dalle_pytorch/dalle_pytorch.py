...
class DALLE(nn.Module):
    ...
    def forward(
        ...
        logits_text = logits[:, :self.text_seq_len, :].contiguous()
        logits_img = logits[:, self.text_seq_len:, :].contiguous()

        loss_text = F.cross_entropy(logits_text.view(-1, logits.size(-1)), labels[:, :self.text_seq_len].contiguous().view(-1))
        loss_img = F.cross_entropy(logits_img.view(-1, logits.size(-1)), labels[:, self.text_seq_len:].contiguous().view(-1))
