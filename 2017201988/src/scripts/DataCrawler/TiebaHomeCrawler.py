...
class Tiezi(object):
    ...
    def get_content_yiyuzheng(self):
        '''解析页面获得所需要的信息'''
        all_text = []
        for i, soup in enumerate(self.soup_):
            # print(soup)
            ...
            if is_yiyu == 1:
                for link in links:
                    if "href" in link.attrs and tiezi_url_re.match(link["href"]):
                        ...
                        tiezi_type.append('抑郁吧')
            if is_yiyuzheng == 1:
                for link in links:
                    if "href" in link.attrs and tiezi_url_re.match(link["href"]):
                        tiezi_list.append(link["href"].split('?')[0]+'?see_lz=1')
                        tiezi_type.append('抑郁症吧')
...
if __name__ == '__main__':
    ...
    filename = 'final_selected_data.xlsx'
    ...
    for index, row in selected_data.iterrows():
        ...
        with tqdm(total=len(tiezi_list)) as pbar:
            ...
            for url in tiezi_list:
                tiezi = Tiezi(url)
                if tiezi_type[tiezi_count] == "抑郁吧":
                    contents = tiezi.get_content_yiyu()
                    for content in contents:
                    ...
                    pbar.update(1)
                elif tiezi_type[tiezi_count] == "抑郁症吧":
                    contents = tiezi.get_content_yiyuzheng()
                    for content in contents:
                        post_time = datetime.strptime(content['post_time'], '%Y-%m-%d %H:%M')
                        if post_time < base_time_former or post_time > base_time_late:
                            continue
