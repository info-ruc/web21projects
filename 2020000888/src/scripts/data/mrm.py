...
def _get_img_tgt_mask(img_mask, txt_len):
    # masked token set to 1 and the not masked token and text tokens are 0.
...
class MrfrDataset(DetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        return (input_ids, img_feat, img_pos_feat,
                attn_masks, img_mask, img_mask_tgt, attn_masks_txt, attn_masks_img)
...
def mrfr_collate(inputs):
    ...
    """
    (input_ids, img_feats, img_pos_feats, attn_masks, img_masks, img_mask_tgts, attn_masks_txt, attn_masks_img
...
class MrcDataset(DetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        return (input_ids, img_feat, img_pos_feat,
                img_soft_labels, attn_masks, img_mask, img_mask_tgt, attn_masks_txt, attn_masks_img)


def mrc_collate(inputs):
    (input_ids, img_feats, img_pos_feats, img_soft_labels,
     attn_masks, img_masks, img_mask_tgts, attn_masks_txt, attn_masks_img) = map(list, unzip(inputs))
...
class FeatDataset(DetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        return (_id, input_ids, img_feat, img_pos_feat,
                img_soft_labels, attn_masks, attn_masks_txt, attn_masks_img, self.lang, example)


def feat_collate(inputs):
    (_ids, input_ids, img_feats, img_pos_feats, img_soft_labels,
     attn_masks, attn_masks_txt, attn_masks_img, langs, examples) = map(list, unzip(inputs))
