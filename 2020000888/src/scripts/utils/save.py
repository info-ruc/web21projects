...
class ModelSaver(object):
    ...
    def save(self, model, step, optimizer=None):
        ...
        last_ckpt = join(self.output_dir, 'last_ckpt')
        if os.path.islink(last_ckpt):
            os.unlink(last_ckpt)
        os.symlink(output_model_file, last_ckpt)
        if optimizer is not None:
            dump = {'step': step, 'optimizer': optimizer.state_dict()}
            if hasattr(optimizer, '_amp_stash'):
                pass  # TODO fp16 optimizer
            output_optim = f'{self.output_dir}/train_state_{step}.{self.suffix}'
            torch.save(dump, output_optim)
            last_optim = join(self.output_dir, 'last_optim')
            if os.path.islink(last_optim):
                os.unlink(last_optim)
            os.symlink(output_optim, last_optim)
