...
class MetaLoader(object):
    ...
    def __init__(self, loaders, accum_steps=1, distributed=False):
        ...
        for n, l in loaders.items():
            ...
            self.sampling_pools.append(n)
            self.probs.append(r)
        self.probs = np.array(self.probs, dtype=np.float32)
        self.probs = self.probs / self.probs.sum()
        print('data/loader.py sampling_pools', self.sampling_pools)
        print('data/loader.py sampling_probs', self.probs)
    ...
    def __iter__(self):
        ...
        while True:
            if self.step % self.accum_steps == 0:
                task = np.random.choice(self.sampling_pools, p=self.probs)
                if self.distributed:
...
def record_cuda_stream(batch):
    if isinstance(batch, torch.Tensor):
        # print("********* cur t is {}".format(batch.shape))
        # print("\t sum: {}".format(torch.sum(batch)))
        # print("\t value: {}".format(batch.data))
